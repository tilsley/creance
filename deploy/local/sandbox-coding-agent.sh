...
# the in-cluster gateway (charts/litellm-gateway). The agent runs as a pod in a locked-down
...
# deep-merges this env over the chart's default env, so JWT_*/REGION/etc. are preserved.
CLAIMS_FILE="$(mktemp -t ca-gw-values.XXXX.yaml)"
printf 'env:\n  CLAIMS_STATIC: '\''{"%s":{"model":"claude-haiku","monthlyBudgetUsd":5}}'\''\n' "$TENANT" > "$CLAIMS_FILE"
helm upgrade --install litellm-gateway charts/litellm-gateway -n "$GW_NS" --create-namespace \
  -f "$CLAIMS_FILE" >/dev/null
rm -f "$CLAIMS_FILE"
kubectl -n "$GW_NS" rollout status deploy/litellm-gateway --timeout=180s
