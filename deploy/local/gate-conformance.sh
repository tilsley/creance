#!/usr/bin/env bash
# Gate conformance suite (ADR-0027): the load-bearing gate contract must hold IDENTICALLY
# across BOTH gateway impls, so the two never drift on R1 (identity) + R2 (budget):
...
echo "▶ start Bun gateway (cheap mode: static token + local gate, scripted model, no AWS)"
...
echo "  un-claimed identity: bun=$bc (cheap mode = flat budget, no claim concept) · lite=$lc (full mode = default-deny 403)"
echo "  → both REJECT, by different mechanism; only the *richness* differs, the deny holds."
